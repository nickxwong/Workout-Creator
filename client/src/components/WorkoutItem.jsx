...
export default function WorkoutItem ({ id, index, exercise, setsReps, workout, setWorkout, moveItem }) {
    ...
    return (
        <div className="workout-item" ref={ref} style={{opacity}} data-handler-id={handlerId}>
            <div className="info" onMouseEnter={() => setShowRemove(true)} onMouseLeave={() => setShowRemove(false)}>
                ...
                {showRemove && <div className="remove-button" onClick={removeExercise}>{trashIcon}</div>}
            </div>
